...
void uoj_sha256_chunk(u8 *chunk, u32 *hs) {
  ...
  u32 w[64];
  for (int i = 0; i < 16; i++) {
    w[i] = ((u32)chunk[i << 2 | 3]) | ((u32)chunk[i << 2 | 2] << 8) |
           ((u32)chunk[i << 2 | 1] << 16) | ((u32)chunk[i << 2] << 24);
  }

  for (int i = 16; i < 64; i++) {
    u32 s0 = uoj_sha2_rotr(w[i - 15], 7) ^ uoj_sha2_rotr(w[i - 15], 18) ^
             (w[i - 15] >> 3);
    u32 s1 = uoj_sha2_rotr(w[i - 2], 17) ^ uoj_sha2_rotr(w[i - 2], 19) ^
             (w[i - 2] >> 10);
    w[i] = w[i - 16] + s0 + w[i - 7] + s1;
  }

  u32 a = hs[0], b = hs[1], c = hs[2], d = hs[3], e = hs[4], f = hs[5],
      g = hs[6], h = hs[7];
  ...
  hs[0] += a, hs[1] += b, hs[2] += c, hs[3] += d, hs[4] += e, hs[5] += f,
      hs[6] += g, hs[7] += h;
}

sha256_t uoj_sha256(int n, u8 *m) {
  u32 hs[] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
...
class uoj_mt_rand_engine {
 ...
 public:
  ...
  uoj_mt_rand_engine(const string &s) {
    ...
    u64 seed = 0;
    for (int i = 0; i < 8; i++) seed = seed << 8 | sum.sum[i];
...
class uoj_secure_io {
 ...
 public:
  ...
  uoj_secure_io() {
    ...
    for (int i = 0; i < (int)sizeof(fake_f); i++) ((u8 *)&fake_f)[i] = rand();
