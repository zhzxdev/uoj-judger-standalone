...
bool put_class_name_to_file(const std::string &fname,
...
struct result {
  ...
  result() = default;
  result(RS_TYPE type, std::string extra, int ust = -1, int usm = -1,
         int exit_code = -1)
...
inline std::ostream &add_runp_arg(std::ostream &out,
                                  const std::pair<T1, std::vector<T2>> &arg) {
...
struct config {
  ...
  config(std::string program_name = "",
  ...
  std::string get_cmd() const {
    ...
    add_runp_arg(sout,
                 std::make_pair("add-readable", this->readable_file_names));
    add_runp_arg(sout,
                 std::make_pair("add-writable", this->writable_file_names));
  ...
  void gen_full_args() {
    ...
    if (type == "java8") {
      full_args[0] =
          get_class_name_from_file(fs::path(full_args[0]) / ".main_class_name");

      std::string jdk = UOJ_JDK8;
      full_args.insert(
          full_args.begin(),
          {fs::canonical(run_path / "runtime" / jdk / "bin" / "java"),
           "-Xmx2048m", "-Xss1024m", "-XX:ActiveProcessorCount=1", "-classpath",
           program_name});
      ...
      full_args.insert(full_args.begin(),
                       {fs::canonical(fs::path(jdk) / "bin" / "java"),
                        "-Xmx2048m", "-Xss1024m", "-XX:ActiveProcessorCount=1",
...
struct pipe_config {
  ...
  pipe_config() = default;
  pipe_config(int _from, int _from_fd, int _to, int _to_fd,
              const std::string &_saving_file_name = "")
      : from(_from),
        from_fd(_from_fd),
        to(_to),
        to_fd(_to_fd),
