...
int execute(const char *cmd) { return system(cmd); }
...
struct PointInfo {
  ...
  PointInfo(const int &_num, const int &_scr, const int &_ust, const int &_usm,
            const string &_info, const string &_in, const string &_out,
            const string &_res)
      : num(_num),
        scr(_scr),
        ust(_ust),
...
struct RunResult {
  ...
  static RunResult from_file(const string &file_name) {
    ...
    if (fres == NULL || fscanf(fres, "%d %d %d %d", &res.type, &res.ust,
                               &res.usm, &res.exit_code) != 4) {
...
void add_subtask_info(const int &num, const int &scr, const string &info,
  ...
  tot_score += scr;
  for (vector<PointInfo>::const_iterator it = points.begin();
       it != points.end(); it++) {
...
struct PipeConfig {
  ...
  PipeConfig() {}
  PipeConfig(int _from, int _from_fd, int _to, int _to_fd,
             const string &_saving_file_name = "")
      : from(_from),
        from_fd(_from_fd),
        to(_to),
        to_fd(_to_fd),
        saving_file_name(_saving_file_name) {}
};
...
RunCheckerResult run_checker(const RunLimit &limit, const string &program_name,
  ...
  RunResult ret = run_program(
      (string(result_path) + "/run_checker_result.txt").c_str(), "/dev/null",
      "/dev/null", (string(result_path) + "/checker_error.txt").c_str(), limit,
      ("--add-readable=" + input_file_name).c_str(),
      ("--add-readable=" + output_file_name).c_str(),
      ("--add-readable=" + answer_file_name).c_str(), program_name.c_str(),
      realpath(input_file_name).c_str(), realpath(output_file_name).c_str(),
      realpath(answer_file_name).c_str(), NULL);
...
RunCompilerResult run_compiler(const char *path, ...) {
  ...
  RunResult ret = vrun_program(
      (result_path + "/run_compiler_result.txt").c_str(), "/dev/null", "stderr",
      (result_path + "/compiler_result.txt").c_str(), RL_COMPILER_DEFAULT,
      argv);
  RunCompilerResult res;
  res.type = ret.type;
  res.ust = ret.ust;
...
RunSimpleInteractionResult run_simple_interation(
  ...
  irpc.limit.real_time = rpc.limit.real_time =
      rpc.limit.time + irpc.limit.time + 1;
...
bool is_illegal_keyword(const string &name) {
  if (name == "__asm" || name == "__asm__" || name == "asm") return true;
  return false;
}
  ...
  while ((c = fgetc(f)) != EOF) {
    if (('0' <= c && c <= '9') || ('a' <= c && c <= 'z') ||
      ...
      if (key.size() < 20)
        ...
        if (is_illegal_keyword(key)) return true;
        key.erase(key.begin());
        key += c;
      }
    } else {
...
RunCompilerResult prepare_java_source(const string &name,
...
RunCompilerResult compile_c(const string &name,
                            const string &path = work_path) {
  return run_compiler(path.c_str(), "/usr/bin/gcc", "-o", name.c_str(), "-x",
                      "c", (name + ".code").c_str(), "-lm", "-O2",
                      "-DONLINE_JUDGE", NULL);
}
RunCompilerResult compile_pas(const string &name,
                              const string &path = work_path) {
  return run_compiler(path.c_str(), "/usr/bin/fpc", (name + ".code").c_str(),
                      "-O2", NULL);
}
RunCompilerResult compile_cpp(const string &name,
                              const string &path = work_path) {
  return run_compiler(path.c_str(), "/usr/bin/g++", "-o", name.c_str(), "-x",
                      "c++", (name + ".code").c_str(), "-lm", "-O2",
                      "-DONLINE_JUDGE", NULL);
}
RunCompilerResult compile_cpp11(const string &name,
                                const string &path = work_path) {
  return run_compiler(path.c_str(), "/usr/bin/g++", "-o", name.c_str(), "-x",
                      "c++", (name + ".code").c_str(), "-lm", "-O2",
...
RunCompilerResult compile_python2(const string &name,
                                  const string &path = work_path) {
  return run_compiler(
      path.c_str(), "/usr/bin/python2", "-E", "-s", "-B", "-O", "-c",
...
RunCompilerResult compile_python3(const string &name,
                                  const string &path = work_path) {
  return run_compiler(
      path.c_str(), "/usr/bin/python3", "-I", "-B", "-O", "-c",
      ("import py_compile\nimport sys\ntry:\n    py_compile.compile('" + name +
       ".code'" + ", '" + name +
       "', doraise=True)\n    sys.exit(0)\nexcept Exception as e:\n    "
       "print(e)\n    sys.exit(1)")
...
RunCompilerResult compile_java8(const string &name,
                                const string &path = work_path) {
  ...
  executef("rm %s/%s -rf 2>/dev/null; mkdir %s/%s", path.c_str(), name.c_str(),
           path.c_str(), name.c_str());
  executef("echo package %s\\; | cat - %s/%s.code >%s/%s/%s.java", name.c_str(),
           path.c_str(), name.c_str(), path.c_str(), name.c_str(),
           main_class.c_str());

  return run_compiler((path + "/" + name).c_str(),
                      "/usr/lib/jvm/java-8-openjdk-amd64/bin/javac",
                      (main_class + ".java").c_str(), NULL);
}
RunCompilerResult compile_java11(const string &name,
                                 const string &path = work_path) {
  RunCompilerResult ret = prepare_java_source(name, path);
  if (!ret.succeeded) return ret;
...
RunCompilerResult compile_c_with_implementer(const string &name,
                                             const string &path = work_path) {
  return run_compiler(path.c_str(), "/usr/bin/gcc", "-o", name.c_str(),
                      "implementer.c", "-x", "c", (name + ".code").c_str(),
                      "-lm", "-O2", "-DONLINE_JUDGE", NULL);
}
RunCompilerResult compile_pas_with_implementer(const string &name,
                                               const string &path = work_path) {
  executef("cp %s %s", (path + "/" + name + ".code").c_str(),
           (path + "/" + conf_str(name + "_unit_name") + ".pas").c_str());
  return run_compiler(path.c_str(), "/usr/bin/fpc", "implementer.pas",
                      ("-o" + name).c_str(), "-O2", NULL);
}
RunCompilerResult compile_cpp_with_implementer(const string &name,
                                               const string &path = work_path) {
  return run_compiler(path.c_str(), "/usr/bin/g++", "-o", name.c_str(),
                      "implementer.cpp", "-x", "c++", (name + ".code").c_str(),
                      "-lm", "-O2", "-DONLINE_JUDGE", NULL);
}
RunCompilerResult compile_cpp11_with_implementer(
...
RunCompilerResult compile_python3(const string &name, const string &path =
work_path) { return run_compiler(path.c_str(),
                        "/usr/bin/python3", "-I", "-B", "-O", "-c", ("import
py_compile\nimport sys\ntry:\n    py_compile.compile('" + name + ".code'" + ",
'" + name + "', doraise=True)\n    sys.exit(0)\nexcept Exception as e:\n
...
struct TestPointConfig {
  ...
  TestPointConfig() : submit_answer(-1), validate_input_before_test(-1) {}
...
PointInfo test_point(const string &name, const int &num,
  ...
  if (!conf_is("interaction_mode", "on")) {
    ...
    if (!tpc.submit_answer) {
      pro_ret = run_submission_program(tpc.input_file_name.c_str(),
                                       tpc.output_file_name.c_str(),
                                       conf_run_limit(num, RL_DEFAULT), name);
      if (conf_has("token")) {
    ...
    RunCheckerResult chk_ret = run_checker(
        conf_run_limit("checker", num, RL_CHECKER_DEFAULT), conf_str("checker"),
    ...
    RunSimpleInteractionResult rires = run_simple_interation(
        ...
        real_output_file_name, conf_run_limit(num, RL_DEFAULT),
...
void main_judger_init(int argc, char **argv) {
  ...
  executef("cp %s/require/* %s 2>/dev/null", data_path.c_str(),
           work_path.c_str());
