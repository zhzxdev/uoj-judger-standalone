...
void file_replace_tokens(const string &name, const string &token,
  ...
  executef("sed -i s/%s/%s/g %s", esc_token.c_str(), esc_new_token.c_str(),
           esc_name.c_str());
}
void file_copy(const string &a, const string &b) {  // copy a to b
  string esc_a = escapeshellarg(a);
  string esc_b = escapeshellarg(b);
  executef("cp %s %s -f", esc_a.c_str(),
           esc_b.c_str());  // the most cubao implementation in the world
}
void file_move(const string &a, const string &b) {  // move a to b
  string esc_a = escapeshellarg(a);
  string esc_b = escapeshellarg(b);
  executef("mv %s %s", esc_a.c_str(),
           esc_b.c_str());  // the most cubao implementation in the world
}
...
const RunLimit RL_JUDGER_DEFAULT =
    RunLimit(600, 2048, 128);  // 2048 = 2GB. change it if needed
...
struct InfoBlock {
  ...
  static InfoBlock from_file_with_size(const string &title,
                                       const string &name) {
    InfoBlock info;
    info.title = title;
    info.content = file_preview(name);
...
enum SCORE_MODE { SM_INT, SM_REAL };
...
#define SCORE_OP(x)                                                          \
  friend inline score_t operator x(const score_t &lhs, const score_t &rhs) { \
    return lhs.__scr x rhs.__scr;                                            \
  }
  SCORE_OP(+)
  SCORE_OP(-)
  SCORE_OP(*)
...
#define SCORE_OP(x)                                                       \
  friend inline bool operator x(const score_t &lhs, const score_t &rhs) { \
    return lhs.__scr x rhs.__scr;                                         \
  }
  SCORE_OP(==)
  SCORE_OP(!=)
...
#define SCORE_OP(x)                                \
  inline score_t &operator x(const score_t &rhs) { \
    __scr x rhs.__scr;                             \
    return *this;                                  \
  }
  SCORE_OP(+=)
...
/**
 * @brief given a score in a system where the full mark is 100, scale it so that
 * the full mark equals "full"
 */
...
struct PointInfo {
  ...
  PointInfo(const int &_num, const score_t &_scr, const int &_ust,
            const int &_usm, const string &_info = "default")
      : num(_num), scr(_scr), ust(_ust), usm(_usm), info(_info) {
    use_li = true;
    if (info == "default") {
      if (scr == 0) {
        info = "Wrong Answer";
  ...
  PointInfo(const int &_num, const score_t &_scr, const int &_ust,
            const int &_usm, const string &_info, const string &_in,
            const string &_out, const string &_res)
      : num(_num),
        scr(_scr),
        ust(_ust),
        usm(_usm),
  ...
  inline bool resolve_dependencies(const map<int, SubtaskInfo> &subtasks) {
    for (const auto &p : meta.subtask_dependencies) {
      ...
      if (meta.subtask_type == "packed") {
        ...
        unrescaled_min_score =
            min(unrescaled_min_score, dep.unrescaled_min_score);
...
struct RunCheckerResult {
  ...
  static RunCheckerResult from_file(const string &file_name,
                                    const RunResult &rres) {
...
void print_config() {
  for (map<string, string>::iterator it = config.begin(); it != config.end();
       ++it) {
...
SubtaskMetaInfo conf_subtask_meta_info(string pre, const int &num) {
  ...
  int endI = num < nT ? conf_int(pre + "subtask_end", num, 0)
                      : conf_int(pre + "n_tests", 10);
  for (int i = startI; i <= endI; i++) {
    meta.points_id.push_back(i);
  }

  meta.subtask_type = conf_str(pre + "subtask_type", num, "packed");
  meta.subtask_used_time_type =
      conf_str(pre + "subtask_used_time_type", num, "sum");
...
struct RunProgramConfig {
  ...
  RunProgramConfig() {
    ...
    for (vector<string>::iterator it = readable_file_names.begin();
  ...
  void set_argv(const char *program_name, ...) {
    ...
    for (const char *arg = va_arg(vl, const char *); arg;
  ...
  string get_cmd() const {
    ...
    for (vector<string>::const_iterator it = readable_file_names.begin();
         it != readable_file_names.end(); it++) {
      sout << " "
           << "--add-readable=" << escapeshellarg(*it);
    }
    for (vector<string>::const_iterator it = writable_file_names.begin();
         it != writable_file_names.end(); it++) {
      sout << " "
           << "--add-writable=" << escapeshellarg(*it);
    }
    for (vector<string>::const_iterator it = argv.begin(); it != argv.end();
         it++) {
...
RunResult vrun_program(const char *run_program_result_file_name,
                       const char *input_file_name,
                       const char *output_file_name,
                       const char *error_file_name, const RunLimit &limit,
...
RunResult run_program(const char *run_program_result_file_name,
  ...
  va_end(vl);
  return vrun_program(run_program_result_file_name, input_file_name,
                      output_file_name, error_file_name, limit, argv);
}

RunValidatorResult run_validator(const string &input_file_name,
                                 const RunLimit &limit,
                                 const string &program_name) {
  RunResult ret =
      run_program((string(result_path) + "/run_validator_result.txt").c_str(),
                  input_file_name.c_str(), "/dev/null",
                  (string(result_path) + "/validator_error.txt").c_str(), limit,
                  ("--type=" + conf_str("val_run_type", "default")).c_str(),
                  program_name.c_str(), NULL);
...
RunCheckerResult run_checker(const RunLimit &limit, const string &program_name,
                             const string &input_file_name,
                             const string &output_file_name,
                             const string &answer_file_name) {
  RunResult ret = run_program(
      (result_path + "/run_checker_result.txt").c_str(), "/dev/null",
      "/dev/null", (result_path + "/checker_error.txt").c_str(), limit,
      ...
      realpath(output_file_name).c_str(), realpath(answer_file_name).c_str(),
      NULL);
...
RunResult run_submission_program(const string &input_file_name,
                                 const string &output_file_name,
                                 const RunLimit &limit, const string &name,
 ...
 * For the current version, it only moves the std to the work dir if it has not
 * been done yet.
 *
 * @param prepared if it is not -1, set the internal variable as if the
 * preparation operations have been done or not (depending on prepared is
 * non-zero or not)
 */
 ...
 * For the current version, it only moves the interactor to the work dir if it
...
RunSimpleInteractionResult run_simple_interaction(
    ...
    const RunLimit &limit, const RunLimit &ilimit, const string &name,
    RunProgramConfig rpc = RunProgramConfig(),
  ...
  irpc.limit = ilimit;
  irpc.set_argv("interactor", input_file_name.c_str(), "/dev/stdin",
                answer_file_name.c_str(), NULL);
  irpc.type = conf_str("interactor_run_type", "default");

  rpc.limit.real_time = rpc.limit.time + irpc.limit.time + 1;
  irpc.limit.real_time = rpc.limit.real_time +
                         1;  // one more second to prevent interactor from TLE
  ...
  RunCheckerResult ires = RunCheckerResult::from_file(
      irpc.error_file_name, RunResult::from_file(irpc.result_file_name));
...
// @deprecated
// will be removed in the future
RunResult run_standard_program(const string &input_file_name,
                               const string &output_file_name,
                               const RunLimit &limit,
                               RunProgramConfig rpc = RunProgramConfig()) {
  prepare_run_standard_program();
  rpc.result_file_name = result_path + "/run_standard_program.txt";
  return run_submission_program(input_file_name, output_file_name, limit, "std",
                                rpc);
}
...
run_compiler_result compile_submission_program(const string &name) {
  return !conf_is("with_implementer", "on") ? compile(name)
                                            : compile_with_implementer(name);
}
...
struct TestPointConfig {
  ...
  void auto_complete(int num) {
    ...
    if (disable_program_input == -1) {
      disable_program_input =
          conf_str("disable_program_input", num, "off") == "on";
    }
...
PointInfo test_point(const string &name, const int &num,
                     TestPointConfig tpc = TestPointConfig()) {
  tpc.auto_complete(num);

  if (tpc.validate_input_before_test) {
    RunValidatorResult val_ret =
        run_validator(tpc.input_file_name,
                      conf_run_limit("validator", 0, RL_VALIDATOR_DEFAULT),
                      conf_str("validator"));
    if (val_ret.type != runp::RS_AC) {
      return PointInfo(num, 0, -1, -1, "Validator " + info_str(val_ret.type),
  ...
  if (!conf_is("interaction_mode", "on")) {
    ...
    if (tpc.checker == "nonempty") {
      ...
      if (usrout == "") {
        return PointInfo(num, 0, -1, -1, "default",
                         file_preview(tpc.input_file_name), usrout,
                         "wrong answer empty file\n");
      } else {
        return PointInfo(num, 100, -1, -1, "default",
                         file_preview(tpc.input_file_name), usrout,
                         "ok nonempty file\n");
      }
    } else {
      ...
      return PointInfo(num, chk_ret.scr, pro_ret.ust, pro_ret.usm, "default",
                       file_preview(tpc.input_file_name),
                       file_preview(tpc.output_file_name), chk_ret.info);
    }
  } else {
    ...
    RunSimpleInteractionResult rires = run_simple_interaction(
        tpc.input_file_name, tpc.answer_file_name, real_input_file_name,
        real_output_file_name, tpc.limit,
        conf_run_limit("interactor", num, RL_INTERACTOR_DEFAULT), name);
    ...
    if (rires.res.type != runp::RS_AC) {
      return PointInfo(num, 0, -1, -1, info_str(rires.res.type),
                       file_preview(real_input_file_name),
                       file_preview(real_output_file_name), "");
    }

    return PointInfo(num, rires.ires.scr, rires.res.ust, rires.res.usm,
                     "default", file_preview(real_input_file_name),
...
PointInfo test_hack_point(const string &name, TestPointConfig tpc) {
  ...
  RunValidatorResult val_ret = run_validator(
      tpc.input_file_name, conf_run_limit("validator", 0, RL_VALIDATOR_DEFAULT),
      conf_str("validator"));
  if (val_ret.type != runp::RS_AC) {
    return PointInfo(0, 0, -1, -1, "Validator " + info_str(val_ret.type),
                     file_preview(tpc.input_file_name), "", "");
  } else if (!val_ret.succeeded) {
    return PointInfo(0, 0, -1, -1, "Invalid Input",
                     file_preview(tpc.input_file_name), "", val_ret.info);
  }
  ...
  if (!conf_is("interaction_mode", "on")) {
    ...
    RunResult std_ret = run_submission_program(
        ...
        conf_run_limit("standard", 0, default_std_run_limit), "std", rpc);
    ...
    RunSimpleInteractionResult rires = run_simple_interaction(
        ...
        conf_run_limit("interactor", 0, RL_INTERACTOR_DEFAULT), "std", rpc);
...
CustomTestInfo ordinary_custom_test(const string &name,
                                    CustomTestConfig ctc = CustomTestConfig()) {
  ...
  RunResult pro_ret =
      run_submission_program(input_file_name, output_file_name, lim, name);
  if (conf_has("token")) {
  ...
  return CustomTestInfo(pro_ret.ust, pro_ret.usm, info, exp,
                        file_preview(output_file_name, 2048));
}
...
template <typename TP>
bool primary_data_test(TP test_point_func, const PrimaryDataTestConfig &pdtc =
                                               PrimaryDataTestConfig()) {
...
void judger_init(int argc, char **argv) {
  ...
  if (config.count("use_builtin_checker")) {
    config["checker"] =
        main_path + "/builtin/checker/" + config["use_builtin_checker"];
  } else {
