...
class random_t {
 ...
 public:
  ...
  double next() {
    return (double)(((long long)(nextBits(26)) << 27) + nextBits(27)) /
           (double)(1LL << 53);
  }
...
NORETURN void InStream::quit(TResult result, const char* msg) {
  ...
  if (ouf.file) fclose(ouf.file);
  if (ans.file) fclose(ans.file);
...
long long InStream::readLong(long long minv, long long maxv,
  ...
  if (result < minv || result > maxv) {
    if (variableName.empty())
      ...
      quit(_wa, ("Integer parameter [name=" + variableName + "] equals to " +
...
double InStream::readReal(double minv, double maxv,
  ...
  if (result < minv || result > maxv) {
    if (variableName.empty())
      ...
      quit(_wa, ("Double parameter [name=" + variableName + "] equals to " +
                 vtos(result) + ", violates the range [" + vtos(minv) + ", " +
                 vtos(maxv) + "]")
                    .c_str());
  }
...
double InStream::readStrictReal(double minv, double maxv,
  ...
  if (result < minv || result > maxv) {
    if (variableName.empty())
      ...
      quit(_wa, ("Strict double parameter [name=" + variableName +
                 "] equals to " + vtos(result) + ", violates the range [" +
                 vtos(minv) + ", " + vtos(maxv) + "]")
                    .c_str());
  }
...
bool InStream::eof() {
  if (!strict && NULL == file) return true;
...
void InStream::readStringTo(std::string& result) {
  if (NULL == file) quit(_pe, "Expected line");
...
NORETURN void __testlib_help() {
  ...
  std::fprintf(stderr, "TESTLIB %s, http://code.google.com/p/testlib/ ",
               VERSION);
