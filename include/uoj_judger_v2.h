...
inline string gen_token() {
  ...
  if (f) {
    for (int i = 0; i < 8; i++) seed = seed << 8 | (u8)fgetc(f);
    fclose(f);
  }
...
inline bool file_encrypt(const string &fi, const string &fo,
                         const string &key) {
...
inline bool file_decrypt(const string &fi, const string &fo,
                         const string &key) {
  string m = file_get_contents(fi);
  uoj_cipher cipher(key);
  if (cipher.decrypt(m)) {
...
bool file_replace_tokens(const string &name, const string &token,
  ...
  return executef("sed -i s/%s/%s/g %s", esc_token.c_str(),
                  esc_new_token.c_str(), esc_name.c_str()) == 0;
}
bool file_copy(const string &a, const string &b) {  // copy a to b
  string esc_a = escapeshellarg(a);
  string esc_b = escapeshellarg(b);
  return executef("cp %s %s -f 2>/dev/null", esc_a.c_str(), esc_b.c_str()) ==
         0;  // the most cubao implementation in the world
}
bool file_hardlink(const string &a, const string &b) {  // copy a to b
  string esc_a = escapeshellarg(a);
  string esc_b = escapeshellarg(b);
  return executef("cp %s %s -lf 2>/dev/null", esc_a.c_str(), esc_b.c_str()) ==
         0;  // the most cubao implementation in the world
}
bool file_move(const string &a, const string &b) {  // move a to b
  string esc_a = escapeshellarg(a);
  string esc_b = escapeshellarg(b);
  return executef("mv %s %s", esc_a.c_str(), esc_b.c_str()) ==
         0;  // the most cubao implementation in the world
}
...
const runp::limits_t RL_JUDGER_DEFAULT(600, 10 * 1024,
                                       128);  // 10GB. change it if needed
...
struct run_checker_result {
  ...
  static run_checker_result from_file(const string &file_name,
                                      const runp::result &rres) {
...
class jgf_error : exception {
 ...
 public:
  explicit jgf_error(const string &code, const string &info = "")
      : code(code), info(info) {
    if (this->info.empty()) {
...
string conf_file_name_with_num(string s, int num) {
  ...
  name << conf_str(s + "_pre", s) << abs(num) << "."
       << conf_str(s + "_suf", "txt");
  return name.str();
}
string conf_input_file_name(int num) {
  ...
  name << conf_str("input_pre", "input") << abs(num) << "."
       << conf_str("input_suf", "txt");
  return name.str();
}
string conf_output_file_name(int num) {
  ...
  name << conf_str("output_pre", "output") << abs(num) << "."
       << conf_str("output_suf", "txt");
  return name.str();
}
runp::limits_t conf_run_limit(string pre, const int &num,
                              const runp::limits_t &val) {
  if (!pre.empty()) {
    pre += "_";
  }
...
void add_subtask_info(const int &num, const int &scr, const string &info,
                      const vector<PointInfo> &points) {
...
result cp(const config &rpc) {
  ...
  if (file_hardlink(rpc.rest_args[0], rpc.rest_args[1]) ||
      file_copy(rpc.rest_args[0], rpc.rest_args[1])) {
    res.exit_code = 0;
    res.type = RS_AC;
  } else {
    res.exit_code = -1;
...
result hide_token(const config &rpc) {
  ...
  int ret = file_hide_token(conf_str("token", ""), rpc.rest_args[0],
                            rpc.rest_args[1]);
  if (ret == -1) {
    res.exit_code = -1;
    res.type = RS_RE;
...
map<string, function<result(const config &)>> call_table = {
    {"nonempty", nonempty}, {"cp", cp}, {"hide_token", hide_token}};
}  // namespace internal
...
// run_xxx will fill in in/out/err/res file names, and may reset args, readable
// files, etc.
run_validator_result run_validator(runp::config rpc,
...
run_checker_result run_checker(runp::config rpc, const string &input_file_name,
  ...
  rpc.readable_file_names.insert(
      rpc.readable_file_names.end(),
      {input_file_name, output_file_name, answer_file_name});
  rpc.rest_args = {fs::canonical(input_file_name),
                   fs::canonical(output_file_name),
                   fs::canonical(answer_file_name)};
  return run_checker_result::from_file(rpc.error_file_name, runp::run(rpc));
}
...
runp::result run_submission_program(runp::config rpc,
                                    const string &input_file_name,
                                    const string &output_file_name) {
...
run_simple_interaction_result run_simple_interaction(
    const string &input_file_name, const string &answer_file_name,
    const string &real_input_file_name, const string &real_output_file_name,
    runp::config rpc, runp::config irpc) {
  prepare_interactor();
  ...
  irpc.limits.real_time = rpc.limits.real_time =
      rpc.limits.time + irpc.limits.time + 1;
  ...
  // from:fd, to:fd
  ric.pipes.push_back(
      runp::interaction::pipe_config(2, 1, 1, 0, real_input_file_name));
  ric.pipes.push_back(
      runp::interaction::pipe_config(1, 1, 2, 0, real_output_file_name));

  runp::interaction::run(ric);

  run_simple_interaction_result rires;
  runp::result res = runp::result::from_file(rpc.result_file_name);
  run_checker_result ires = run_checker_result::from_file(
      irpc.error_file_name, runp::result::from_file(irpc.result_file_name));
...
run_transformer_result transform_file(runp::config rpc, const string &in,
                                      const string &out) {
...
run_compiler_result compile(const string &name) {
  ...
  runp::config rpc(
      main_path / "run" / "compile",
      {"--custom", main_path / "run" / "runtime", "--lang", lang, name});
  rpc.limits = RL_COMPILER_DEFAULT;
  return run_compiler(rpc);
}

run_compiler_result compile_with_implementer(
    const string &name, const string &implementer = "implementer") {
  string lang = conf_str(name + "_language");
  if (conf_has(name + "_unit_name")) {
    file_put_contents(work_path / (name + ".unit_name"),
                      conf_str(name + "_unit_name"));
  }
  runp::config rpc(main_path / "run" / "compile",
                   {"--custom", main_path / "run" / "runtime", "--impl",
                    implementer, "--lang", lang, name});
  rpc.limits = RL_COMPILER_DEFAULT;
  return run_compiler(rpc);
}
...
conf_is("validate_input_before_test", "on");
                }
                if (input_file_name.empty()) {
                        input_file_name = data_path + "/" +
conf_input_file_name(num);
                }
                if (output_file_name.empty()) {
                        output_file_name = work_path + "/" +
conf_output_file_name(num);
                }
                if (answer_file_name.empty()) {
                        answer_file_name = data_path + "/" +
conf_output_file_name(num);
                }
        }
};
...
class TestPoint {
 public:
  ...
  enum { AUTHOR_SETTER = 0, AUTHOR_STD };
  enum { AUTHOR_USER = AUTHOR_USERBIT, AUTHOR_USER2 };
  ...
  map<Author, string> author_name{{AUTHOR_SETTER, "Problem Setter"},
...
fs::path TestPoint::get_fname(const string &fkey, const string &default_val) {
  return fname.count(fkey) ? fname[fkey]
                           : fs::path(conf_str(fkey, default_val));
}
...
runp::config TestPoint::get_program(const string &id) {
  try {
    ...
    throw jgf_error("PROGNF1",
                    "Program " + id + " Not Found");  // program not found
  }
}
runp::config TestPoint::search_program(const initializer_list<string> &ids) {
  ...
  throw jgf_error("PROGNF2", "Program " + list_to_string(ids) +
...
bool TestPoint::add_info_block(const string &fkey) {
  if (ftitle.count(fkey) && ftitle[fkey] != "disabled") {
    po.li.push_back(InfoBlock::from_file_with_size(ftitle[fkey], fname[fkey],
                                                   get_fpreview_size(fkey)));
  }
  return true;
}
...
bool TestPoint::preprocess_input(const string &input) {
  return validate(input) && check() && encode(input) && check() &&
         encrypt(input) && check();
}

bool TestPoint::postprocess_output(const string &output) {
  return decrypt(output) && check() && decode(output) && check();
}
...
bool TestPoint::validate(const string &fkey) {
  ...
  if (ret.type != runp::RS_AC) {
    throw jgf_error(
        "VALFAL",
        "Validator " + runp::rstype_str(ret.type));  // validator failed
...
bool TestPoint::encode(const string &input) {
  run_transformer_result ret =
      transform_file(search_program({input + "encoder", "encoder"}),
                     fname[input], fname[input + ".plain"]);
  add_info_block(input + ".plain");
  if (ret.type != runp::RS_AC) {
    throw jgf_error("ECOFAL",
                    "Encoder " + runp::rstype_str(ret.type));  // encoder failed
  } else if (!ret.succeeded) {
    set_res_if_empty(ret.info);
...
bool TestPoint::encrypt(const string &input) {
  run_transformer_result ret =
      transform_file(search_program({input + "encrypter", "encrypter"}),
                     fname[input + ".plain"], fname[input + ".raw"]);
  add_info_block(input + ".raw");
  if (ret.type != runp::RS_AC) {
    throw jgf_error(
        "ECRFAL",
        "Encrypter " + runp::rstype_str(ret.type));  // encrypter failed
  } else if (!ret.succeeded) {
    set_res_if_empty(ret.info);
    set_info_if_empty("Invalid Input", fauthor[input]);
    if (!(fauthor[input] & AUTHOR_USERBIT)) {
...
bool TestPoint::decrypt(const string &output) {
  run_transformer_result ret =
      transform_file(search_program({output + "decrypter", "decrypter"}),
                     fname[output + ".raw"], fname[output + ".plain"]);
  add_info_block(output + ".plain");
  if (ret.type != runp::RS_AC) {
    throw jgf_error(
        "DCRFAL",
        "Decrypter " + runp::rstype_str(ret.type));  // decrypter failed
  } else if (!ret.succeeded) {
    set_res_if_empty(ret.info);
...
bool TestPoint::decode(const string &output) {
  run_transformer_result ret =
      transform_file(search_program({output + "decoder", "decoder"}),
                     fname[output + ".plain"], fname[output]);
  add_info_block(output);
  if (ret.type != runp::RS_AC) {
    throw jgf_error(
        "DCOFAL", "Decrypter " + runp::rstype_str(ret.type));  // decoder failed
  } else if (!ret.succeeded) {
    set_res_if_empty(ret.info);
    if (!(fauthor[output] & AUTHOR_USERBIT)) {
      set_info_if_empty("Invalid Output", fauthor[output]);
...
bool TestPoint::generate_answer(const string &input, const string &answer) {
  runp::result ret = run_submission_program(
      get_program("std"), fname["input.raw"], fname["answer.raw"]);
  if (ret.type != runp::RS_AC) {
...
bool TestPoint::grade(const string &input, const string &output,
                      const string &answer) {
  run_checker_result ret = run_checker(get_program("chk"), fname[input],
                                       fname[output], fname[answer]);
  if (ret.type != runp::RS_AC) {
...
bool TestPoint::run_custom(const string &id, const string &input,
                           const string &output) {
  if (!program.count(id)) {
  ...
  runp::result ret = run_submission_program(
      get_program(id), fname[input + ".raw"], fname[output + ".raw"]);
...
void TestPoint::complete_config() {
  map_add(fauthor, {{"input", AUTHOR_SETTER},
                    {"output", AUTHOR_USER},
                    {"answer", AUTHOR_SETTER}});

  map_add(fname, {{"input", data_path / conf_input_file_name(num)},
  ...
  if (get_fname("input_encoder", "cp") == "cp") {
    map_add(ftitle, {{"input", "input"}, {"output.plain", "output"}});
  } else {
    map_add(ftitle, {{"input", "data_for_input_generation"},
                     {"input.plain", "input"},
                     {"output.plain", "output"}});
  }
}
...
void TestPoint::complete_hack_config() {
  map_add(fauthor, {{"input", AUTHOR_USER},
                    {"output", AUTHOR_USER},
                    {"answer", AUTHOR_STD}});

  map_add(fname, {{"input", work_path / "hack_input.txt"},
                  {"input.plain", work_path / "io" / "plain_input.txt"},
                  {"input.raw", work_path / "io" / "raw_input.txt"},
                  {"output.raw", work_path / "io" / "raw_output.txt"},
                  {"output.plain", work_path / "io" / "plain_output.txt"},
...
void TestPoint::complete_custom_test_config() {
  map_add(fauthor, {{"input", AUTHOR_USER}, {"output", AUTHOR_USER}});

  map_add(fname, {{"input.plain", work_path / "input.txt"},
                  {"input.raw", work_path / "io" / "raw_input.txt"},
                  {"output.raw", work_path / "io" / "raw_output.txt"},
                  {"output.plain", work_path / "io" / "plain_output.txt"}});
  ...
  map_add(fpreview_size, {{"output.plain", 2048}});
}
...
bool TestPoint::_custom_test() {
  return prepare_io() && add_info_block("input") && encrypt("input") &&
         run_custom("answer", "input", "output") && decrypt("output") &&
         check();
}
...
class SubmitAnswerTestPoint : public TestPoint {
 protected:
  ...
  virtual bool _test() override {
    return prepare_io() && add_info_block("input") && encode("input") &&
           check() && decode("output") && check() &&
...
class SingleProgramTestPoint : public TestPoint {
 protected:
  virtual bool run() {
    ...
    runp::result ret = run_submission_program(
        get_program("answer"), fname["input.raw"], fname["output.raw"]);
    if (ret.type != runp::RS_AC) {
      set_info_if_empty(runp::rstype_str(ret.type));
      set_res_if_empty(ret.extra);
      if (ret.type == runp::RS_JGF) {
  ...
  virtual bool _test() override {
    return prepare_io() && add_info_block("input") &&
           preprocess_input("input") && run() && postprocess_output("output") &&
           grade("input", "output", "answer") && check();
  }

  virtual bool _hack_test() override {
    return prepare_io() && add_info_block("input") &&
           preprocess_input("input") && generate_answer("input", "answer") &&
           postprocess_output("answer") && run() &&
           postprocess_output("output") && grade("input", "output", "answer") &&
           check();
  }
};
...
class SimpleInteractionTestPoint : public SingleProgramTestPoint {
 public:
  enum { AUTHOR_INTERACTOR = 1 << 4 };

 protected:
  virtual void complete_config() override {
    map_add(fauthor, {{"program_input", AUTHOR_INTERACTOR}});
    map_add(author_name, {{AUTHOR_INTERACTOR, "Interactor"}});
    map_add(fname,
            {{"program_input.raw", work_path / "io" / "raw_program_input.txt"},
             {"program_input.plain",
              work_path / "io" / "plain_program_input.txt"}});

    if (get_fname("input_encoder", "cp") == "cp") {
      map_add(ftitle, {{"input", "input_to_interactor"},
  ...
  virtual bool run() {
    ...
    run_simple_interaction_result rires = run_simple_interaction(
        fname["input.raw"], fname["answer"], fname["program_input.raw"],
        fname["output.raw"], get_program("answer"), get_program("interactor"));
  ...
  virtual bool _test() override {
    return prepare_io() && add_info_block("input") &&
           preprocess_input("input") && run() && decrypt("program_input") &&
           decrypt("output") && check();
  }
...
/*
PointInfo test_point(const string &name, const int &num, TestPointConfig tpc =
TestPointConfig()) { tpc.auto_complete(num);

        if (tpc.validate_input_before_test) {
                RunValidatorResult val_ret = run_validator(
                                tpc.input_file_name,
                                conf_run_limit("validator", 0,
RL_VALIDATOR_DEFAULT), conf_str("validator")); if (val_ret.type != RS_AC) {
                        return PointInfo(num, 0, -1, -1,
                                        "Validator " +
runp::rstype_str(val_ret.type), file_preview(tpc.input_file_name), "",
                                        "");
        ...
        if (!conf_is("interaction_mode", "on")) {
                ...
                if (!tpc.submit_answer) {
                        pro_ret = run_submission_program(
                                        ...
                                        name);
                        if (conf_has("token")) {
                                file_hide_token(tpc.output_file_name,
conf_str("token", ""));
                        }
...
file_preview(tpc.output_file_name),
                ...
                RunCheckerResult chk_ret = run_checker(
                                conf_run_limit("checker", num,
RL_CHECKER_DEFAULT), conf_str("checker"), tpc.input_file_name,
                                tpc.output_file_name,
                                tpc.answer_file_name);
                if (chk_ret.type != RS_AC) {
                        return PointInfo(num, 0, -1, -1,
                                        "Checker " +
runp::rstype_str(chk_ret.type), file_preview(tpc.input_file_name),
file_preview(tpc.output_file_name),
                                        "");
                }
                ...
                return PointInfo(num, chk_ret.scr, pro_ret.ust, pro_ret.usm,
                                "default",
                                file_preview(tpc.input_file_name),
file_preview(tpc.output_file_name), chk_ret.info); } else { string
real_output_file_name = tpc.output_file_name + ".real_input.txt"; string
real_input_file_name = tpc.output_file_name + ".real_output.txt";
                RunSimpleInteractionResult rires = run_simple_interaction(
                                ...
                                conf_run_limit(num, RL_DEFAULT),
                                conf_run_limit("interactor", num,
RL_INTERACTOR_DEFAULT), name);

                if (rires.ires.type != RS_AC) {
                        return PointInfo(num, 0, -1, -1,
                                        "Interactor " +
runp::rstype_str(rires.ires.type), file_preview(real_input_file_name),
file_preview(real_output_file_name),
                                        "");
                }
...
file_preview(real_output_file_name),
                ...
                return PointInfo(num, rires.ires.scr, rires.res.ust,
rires.res.usm, "default", file_preview(real_input_file_name),
file_preview(real_output_file_name), rires.ires.info);
        }
}*/
...
PointInfo test_hack_point(const string &name, TestPointConfig tpc) {
        ...
        if (!conf_is("interaction_mode", "on")) {
                runp::config rpc;
                rpc.result_file_name = result_path +
"/run_standard_program.txt"; runp::result std_ret = run_submission_program(
                                tpc.input_file_name,
                                tpc.answer_file_name,
                                conf_run_limit("standard", 0,
default_std_run_limit), "std", rpc); if (std_ret.type != RS_AC) { return
PointInfo(0, 0, -1, -1, "Standard Program " + runp::rstype_str(std_ret.type),
                                        file_preview(tpc.input_file_name), "",
                                        "");
                }
                if (conf_has("token")) {
...
""));
                ...
                runp::config rpc;
                rpc.result_file_name = result_path +
"/run_standard_program.txt"; string real_output_file_name =
tpc.answer_file_name; string real_input_file_name = tpc.output_file_name +
".real_output.txt"; RunSimpleInteractionResult rires = run_simple_interaction(
                                tpc.input_file_name,
                                tpc.answer_file_name,
                                real_input_file_name,
                                real_output_file_name,
                                conf_run_limit("standard", 0,
default_std_run_limit), conf_run_limit("interactor", 0, RL_INTERACTOR_DEFAULT),
                                "std",
                                rpc);
                ...
                if (rires.ires.type != RS_AC) {
                        return PointInfo(0, 0, -1, -1,
                                        "Interactor " +
runp::rstype_str(rires.ires.type) + " (Standard Program)",
                                        file_preview(real_input_file_name), "",
                                        "");
                }
                if (rires.res.type != RS_AC) {
                        return PointInfo(0, 0, -1, -1,
                                        "Standard Program " +
runp::rstype_str(rires.res.type), file_preview(real_input_file_name), "",
                                        "");
                }
        }
...
CustomTestInfo ordinary_custom_test(const string &name) {
        ...
        if (pro_ret.type == RS_TLE) {
                exp = "<p>[<strong>time limit:</strong> " + vtos(lim.time) +
"s]</p>";
        }
...
class Judger {
 protected:
  ...
  bool compile_and_add_program(const string &name) {
    run_compiler_result c_ret = !conf_is("with_implementer", "on")
                                    ? compile("answer")
  ...
  virtual void configure_programs_for_point(int num) {
    if (program.count("std")) {
      program["std"].limits =
          conf_run_limit("standard", num, conf_run_limit(num, RL_DEFAULT));
    }
    ...
    for (auto &name : {"encoder", "encrypter", "decrypter", "decoder"}) {
      if (program.count(name)) {
        program[name].limits =
            conf_run_limit(name, num, RL_TRANSFORMER_DEFAULT);
      }
    }
  }
  ...
  virtual void prepare() {
    if (conf_has("use_builtin_checker")) {
      add_program("chk", runp::config(main_path / "builtin" / "checker" /
                                      conf_str("use_builtin_checker")));
    }

    // search for chk, val, std, encoder, decoder, etc.
    for (auto &p : fs::directory_iterator(data_path)) {
      if ((p.status().permissions() & fs::perms::others_exec) !=
          fs::perms::none) {
 ...
 public:
    ...
    if (nT == 0) {  // OI
      for (int i = 1; i <= n; i++) {
        ...
        add_point_info(po);
               ...
               conf_str("subtask_type", 1, "packed") == "packed") {  // ACM
      for (int i = 1; i <= n; i++) {
        report_judge_status_f("Judging Test #%d", i);
        PointInfo po = this->test_point(i);
        if (po.scr != 100) {
      ...
      for (int t = 1; t <= nT; t++) {
        ...
        bool skipped = false;
        for (vector<int>::iterator it = dependences.begin();
  ...
  virtual void custom_test() {
    if (!is_custom_test_enabled()) {
      end_judge_judgment_failed(
          "Custom test is not supported in this problem.");
    } else {
      this->prepare();
...
void main_judger_init(int argc, char **argv) {
  try {
    ...
    if (fs::is_directory(data_path / "require")) {
      fs::copy(data_path / "require", work_path,
