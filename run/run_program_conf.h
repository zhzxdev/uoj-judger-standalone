...
inline bool is_in_set_smart(string name, const set<string> &s) {
  ...
  for (size_t i = 0; i + 1 < name.size(); i++) {
    if ((i == 0 || name[i - 1] == '/') && name[i] == '.' &&
        name[i + 1] == '.' && (i + 2 == name.size() || name[i + 2] == '.')) {
      return false;
    }
  }
  int level;
...
inline bool is_statable_file(const string &name) {
  ...
  return is_in_set_smart(name, statable_file_name_set) ||
         is_in_set_smart(realpath(name), statable_file_name_set);
}
inline bool is_soft_ban_file(const string &name) {
  ...
  return is_in_set_smart(name, soft_ban_file_name_set) ||
         is_in_set_smart(realpath(name), soft_ban_file_name_set);
}

#ifdef __x86_64__
int syscall_max_cnt_list_default[][2] = {{__NR_read, -1},
                                         {__NR_write, -1},
                                         {__NR_readv, -1},
                                         {__NR_writev, -1},
                                         {__NR_open, -1},
...
int syscall_soft_ban_list_default[] = {-1};
...
void init_conf(const RunProgramConfig &config) {
  for (int i = 0; syscall_max_cnt_list_default[i][0] != -1; i++) {
    syscall_max_cnt[syscall_max_cnt_list_default[i][0]] =
        syscall_max_cnt_list_default[i][1];
  }
  ...
  if (config.type != "java8" && config.type != "java11") {
    ...
    if (p == string::npos) {
      ...
      readable_file_name_set.insert(config.work_path + "/" +
                                    config.program_name.substr(0, p) + "/");
    }
  }
  add_file_permission(config.work_path, 'r');

  for (vector<string>::const_iterator it = config.extra_readable_files.begin();
       it != config.extra_readable_files.end(); it++) {
    add_file_permission(*it, 'r');
  }
  for (vector<string>::const_iterator it = config.extra_writable_files.begin();
       it != config.extra_writable_files.end(); it++) {
  ...
  if (config.type == "python2") {
    ...
    readable_file_name_set.insert(
        abspath(0, string(self_path) + "/../runtime") + "/");
#ifdef UOJ_JUDGER_BASESYSTEM_UBUNTU1804
    readable_file_name_set.insert("/sys/fs/cgroup/cpu/");
    readable_file_name_set.insert("/sys/fs/cgroup/cpu,cpuacct/");
    readable_file_name_set.insert("/sys/fs/cgroup/memory/");
...
inline bool on_dgs_file_detect(pid_t pid, user_regs_struct reg,
                               const string &fn) {
