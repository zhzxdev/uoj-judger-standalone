...
argp_option run_program_argp_options[] = {
    ...
    {"type", 't', "TYPE", 0,
     "Set the program type (for some program such as python)", 10},
    {"res", 'r', "RESULT_FILE", 0,
     "Set the file name for outputing the result            ", 10},
    ...
    {"add-readable-raw", 504, "FILE", 0,
     "Add a readable (don't transform to its real path)", 15},
    {"add-writable-raw", 506, "FILE", 0,
     "Add a writable (don't transform to its real path)", 15},
    {0}};
error_t run_program_argp_parse_opt(int key, char *arg,
...
void parse_args(int argc, char **argv) {
  ...
  argp_parse(&run_program_argp, argc, argv, ARGP_NO_ARGS | ARGP_IN_ORDER, 0,
             &run_program_config);
  ...
  // NOTE: program_name is the full path of the program, not just the file name
  // (but can start with "./")
  if (run_program_config.work_path.empty()) {
    ...
    if (run_program_config.work_path.empty() ||
        chdir(run_program_config.work_path.c_str()) == -1) {
      // work path does not exist
...
[[noreturn]] void run_child() {
  ...
  // TODO: use https://man7.org/linux/man-pages/man3/vlimit.3.html to limit
  // virtual memory

  if (run_program_config.input_file_name != "stdin") {
    if (freopen(run_program_config.input_file_name.c_str(), "r", stdin) ==
        NULL) {
      exit(11);
    }
  }
  if (run_program_config.output_file_name != "stdout" &&
    ...
    if (freopen(run_program_config.output_file_name.c_str(), "w", stdout) ==
        NULL) {
      exit(12);
    }
  }
  if (run_program_config.error_file_name != "stderr") {
    if (run_program_config.error_file_name == "stdout") {
      ...
      if (freopen(run_program_config.error_file_name.c_str(), "w", stderr) ==
          NULL) {
        exit(14);
      }
    }
...
// limit for the safe mode, an upper limit for the number of calls to
// fork/vfork/clone
...
bool has_real_TLE() {
  ...
  return elapsed.tv_sec + elapsed.tv_usec / 1'000'000. >=
         run_program_config.limits.real_time;
}
...
string get_usage_summary(struct rusage *rusep) {
  ...
  sout << rusep->ru_utime.tv_sec * 1000 + rusep->ru_utime.tv_usec / 1000
       << "ms / ";
...
void stop_all(runp::result res) {
  ...
  while (true) {
    ...
    // it is possible that a newly created process hasn't been logged into
    // rp_children kill it for safty
    kill(pid, SIGKILL);
  }
...
run_event next_event() {
  ...
  if (WIFEXITED(stat)) {
    if (p == 0) {
      stop_all(runp::result(
          runp::RS_JGF,
          "error code: ZROEX"));  // the 0th child process exited unexpectedly
    }
    e.type = ET_EXIT;
  ...
  if (WIFSIGNALED(stat)) {
    if (p == 0) {
      stop_all(runp::result(runp::RS_JGF,
                            "error code: ZROSIG"));  // the 0th child process
                                                     // signaled unexpectedly
    }
    e.type = ET_SIGNALED;
    e.sig = WTERMSIG(stat);
    return e;
  ...
  if (!WIFSTOPPED(stat)) {
    stop_all(runp::result(
        runp::RS_JGF,
        "error code: NSTOP"));  // expected WIFSTOPPED, but it is not
  }
  ...
  if (e.cp->flags & CPF_STARTUP) {
    ...
    if (ptrace(PTRACE_SETOPTIONS, e.pid, NULL, ptrace_opt) == -1) {
      stop_all(
          runp::result(runp::RS_JGF, "error code: PTRCFAL"));  // ptrace failed
    }
  ...
  switch (e.sig) {
    case SIGTRAP:
      switch (e.pevent) {
        ...
        default:
          stop_all(runp::result(
              runp::RS_JGF, "error code: PTRCSIG"));  // unknown ptrace signal
      }
    case SIGSTOP:
    ...
    case SIGVTALRM:
      ...
      // if the program reaches this line... then something goes wrong (rusage
      // says no TLE, but timer says TLE) just ignore it and wait for another
      // period
      e.sig = 0;
      e.type = ET_RESTART;
      return e;
...
void dispatch_event(run_event &&e) {
  ...
  switch (e.type) {
    ...
    case ET_USER_CPU_TLE:
      stop_all(runp::result(
          runp::RS_TLE, "user CPU time limit exceeded: >" +
                            to_string(run_program_config.limits.time) + "s"));
    case ET_MLE:
      stop_all(runp::result(
          runp::RS_MLE,
          "max RSS >" + to_string(run_program_config.limits.memory) + "MB"));
    case ET_OLE:
      stop_all(runp::result(runp::RS_OLE,
                            "output limit exceeded: >" +
                                to_string(run_program_config.limits.output) +
                                "MB"));
    case ET_EXIT:
      if (run_program_config.need_show_trace_details) {
      ...
      if (rp_children[0].flags & CPF_STARTUP) {
        stop_all(runp::result(
            runp::RS_JGF, "error code: CPCMDER1"));  // rp_children mode error
      } else if (rp_children.size() < 2 ||
                 (rp_children[1].flags & CPF_STARTUP)) {
        stop_all(runp::result(
            runp::RS_JGF, "error code: CPCMDER2"));  // rp_children mode error
      } else {
        if (e.cp == rp_children.data() + 1) {
          stop_all(runp::result(runp::RS_AC,
                                "exit with code " + to_string(e.exitcode),
      ...
      if (e.cp != rp_children.data() + 0 && !run_program_config.unsafe) {
        ...
        if (e.cp->try_to_create_new_process) {
          ...
          if (total_rp_children > MAX_TOTAL_RP_CHILDREN) {
            stop_all(runp::result(
                runp::RS_DGS,
                "the limit on the amount of child processes is exceeded"));
          }
        }
      }
      break;
  ...
  if (ptrace(restart_op, e.pid, NULL, e.sig) < 0) {
    if (errno != ESRCH) {
      stop_all(runp::result(runp::RS_JGF,
                            "error code: PTRESFAL"));  // ptrace restart failed
    }
  }
}
...
[[noreturn]] void trace_children() {
  rp_timer_pid = fork();
  if (rp_timer_pid == -1) {
    runp::result(runp::RS_JGF, "error code: FKFAL2")
        .dump_and_exit();  // fork failed
  } else if (rp_timer_pid == 0) {
    struct timespec ts =
        runp::double_to_timespec(run_program_config.limits.real_time);
...
int main(int argc, char **argv) {
  try {
    ...
    runp::result(runp::RS_JGF, "error code: PTHFAL2")
        .dump_and_exit();  // path failed
  }
