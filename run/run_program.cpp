...
void parse_args(int argc, char **argv) {
  ...
  run_program_config.stack_limit =
      min(run_program_config.stack_limit, run_program_config.memory_limit);
  ...
  if (run_program_config.type == "java8" ||
      run_program_config.type == "java11") {
  ...
  if (run_program_config.work_path.empty()) {
    ...
    run_program_config.program_basename =
        basename(run_program_config.program_name);
  ...
  if (run_program_config.type == "python2") {
    ...
    run_program_config.argv.insert(run_program_config.argv.begin(), pre,
                                   pre + 4);
    ...
    string pre[3] = {"/usr/lib/jvm/java-8-openjdk-amd64/bin/java", "-Xmx1024m",
                     "-Xss1024m"};
    run_program_config.argv.insert(run_program_config.argv.begin(), pre,
                                   pre + 3);
  } else if (run_program_config.type == "java11") {
    string pre[3] = {"/usr/lib/jvm/java-11-openjdk-amd64/bin/java", "-Xmx1024m",
                     "-Xss1024m"};
    run_program_config.argv.insert(run_program_config.argv.begin(), pre,
                                   pre + 3);
  }
...
void run_child() {
  set_limit(RLIMIT_CPU, run_program_config.time_limit,
            run_program_config.real_time_limit);
    ...
    if (WIFSTOPPED(stat)) {
      ...
      if (rp_children[p].mode == -1) {
        if ((p == 0 && sig == SIGTRAP) || (p != 0 && sig == SIGSTOP)) {
          if (p == 0) {
            ...
            if (run_program_config.safe_mode) {
              ptrace_opt |= PTRACE_O_TRACECLONE | PTRACE_O_TRACEFORK |
                            PTRACE_O_TRACEVFORK;
        ...
        if (rp_children[p].mode == 0) {
          if (run_program_config.safe_mode) {
            if (!check_safe_syscall(
                    pid, run_program_config.need_show_trace_details)) {
              stop_all();
              return RunResult(RS_DGS);
            }
          }
