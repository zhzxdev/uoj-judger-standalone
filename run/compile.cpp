...
class fail_to_read_src_error : public std::runtime_error {
 public:
  explicit fail_to_read_src_error(
      const std::string &what =
          "An error occurs when trying to read the source code.")
...
const std::vector<std::pair<const char *, const char *>> suffix_search_list = {
    {".code", ""},         {"20.cpp", "C++20"},   {"17.cpp", "C++17"},
    {"14.cpp", "C++14"},   {"11.cpp", "C++11"},   {".cpp", "C++"},
    {".c", "C"},           {".pas", "Pascal"},    {"2.7.py", "Python2.7"},
    {".py", "Python3"},    {"7.java", "Java7"},   {"8.java", "Java8"},
    {"11.java", "Java11"}, {"14.java", "Java14"}, {"17.java", "Java17"},
};
...
compile_config parse_args(int argc, char **argv) {
  argp_option argp_options[] = {
      ...
      {"custom", 'c', "CUSTOM", 0,
       "path to custom compilers (those are not placed in /usr/bin)", 4},
      {"cinclude", 'I', "DIRECTORY", 0,
       "add the directory dir to the list of directories to be searched for "
       "header files during preprocessing (for C/C++)",
       5},
      {0}};
  char argp_args_doc[] = "name";
  char argp_doc[] = "compile: a tool to compile programs";

  argp compile_argp = {argp_options, compile_argp_parse_opt, argp_args_doc,
                       argp_doc};

  compile_config config;
  argp_parse(&compile_argp, argc, argv, ARGP_NO_ARGS | ARGP_IN_ORDER, 0,
             &config);
    ...
    if (tokens[i] == "class") {
      ...
      for (size_t k = 0; k < name.length(); k++) {
        if (!isalnum(name[k]) && name[k] != '_') {
          throw compile_error(
              "The name of the main class should only contain letters, numbers "
              "and underscore sign.");
        }
      }
      if (!isalpha(name[0])) {
        throw compile_error(
            "The name of the main class cannot begin with a number.");
      }
      return tokens[i + 1];
    }
  }
...
int compile_cpp(const compile_config &conf, const std::string &std) {
  ...
  if (conf.implementer.empty()) {
    return execute(UOJ_GPLUSPLUS, sflags.str(), "-o", conf.name, "-x", "c++",
                   conf.src);
  } else {
    return execute(UOJ_GPLUSPLUS, sflags.str(), "-o", conf.name,
...
int compile_c(const compile_config &conf) {
  ...
  if (conf.implementer.empty()) {
    return execute(UOJ_GCC, sflags.str(), "-o", conf.name, "-x", "c", conf.src);
  } else {
    return execute(UOJ_GCC, sflags.str(), "-o", conf.name,
...
int compile_python2_7(const compile_config &conf) {
  ...
  std::string compiler_code =
      ...
      "try:\n"
      "    py_compile.compile('" +
      conf.src + "', cfile='" + conf.name + "', dfile='" + dfile +
      "', doraise=True)\n"
      "    sys.exit(0)\n"
      "except Exception as e:\n"
      "    print e\n"
      "    sys.exit(1)\n";
  return execute(UOJ_PYTHON2_7, "-E", "-s", "-B", "-O", "-c",
                 escapeshellarg(compiler_code));
}
int compile_python3(const compile_config &conf) {
  ...
  std::string compiler_code =
      ...
      "    print(e)\n"
      "    sys.exit(1)\n";
  return execute(UOJ_PYTHON3, "-I", "-B", "-O", "-c",
                 escapeshellarg(compiler_code));
}
int compile_java(const compile_config &conf, const std::string &jdk) {
  ...
  try {
    ...
    int ret = execute("cd", conf.name, "&&", jdk + "/bin/javac",
                      main_class + ".java");
    fs::remove(fs::path(conf.name) / (main_class + ".java"));
    put_class_name_to_file(fs::path(conf.name) / ".main_class_name",
                           main_class);
    return ret;
  ...
  if (conf.implementer.empty()) {
    ...
    try {
      std::string unit_name =
          get_class_name_from_file(conf.name + ".unit_name");
      if (!unit_name.empty()) {
        fs::copy_file(conf.src, unit_name + ".pas");
      }
      int ret =
          execute(UOJ_FPC, conf.implementer + ".pas", "-o" + conf.name, "-O2");
      if (!unit_name.empty()) {
...
int compile(const compile_config &conf) {
  if ((conf.lang.length() > 0 && conf.lang[0] == 'C') &&
      has_illegal_keywords_in_file(conf.src)) {
