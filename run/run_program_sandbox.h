...
enum EX_CHECK_TYPE : unsigned {
  ...
  ECT_FILEAT_OP =
      ECT_FILE_OP | ECT_END_AT,   // it is a file operation ended with "at"
  ECT_FILE_W = 1 << 3,            // intend to write
  ECT_FILE_R = 1 << 4,            // intend to read
  ECT_FILE_S = 1 << 5,            // intend to stat
  ECT_CHECK_OPEN_FLAGS = 1 << 6,  // check flags to determine whether it is to
                                  // read/write (for open and openat)
  ECT_FILE2_W = 1 << 7,           // intend to write (2nd file)
  ECT_FILE2_R = 1 << 8,           // intend to read  (2nd file)
  ECT_FILE2_S = 1 << 9,           // intend to stat  (2nd file)
  ECT_CLONE_THREAD =
      1
      << 10,  // for clone(). Check that clone is making a non-suspicious thread
  ECT_KILL_SIG0_ALLOWED =
      1 << 11,  // forbid kill but killing with sig0 is allowed
};
...
struct syscall_info {
  ...
  static syscall_info kill_type_syscall(unsigned extra_check = ECT_CNT,
                                        int max_cnt = 0) {
    if (max_cnt != -1) {
      extra_check |= ECT_CNT;
    }
  ...
  for (auto &v : lv) {
    ...
    if (reachable) {
      ...
      if (pos == "/proc/self") {
        ...
        realpos =
            "/proc/" + to_string(get_tgid_from_pid(pid)) + "/" + to_string(pid);
      } else {
...
void init_conf() {
  ...
  if (config.type == "java8") {
    add_file_permission(runp::run_path.string() + "/runtime/" + UOJ_JDK8 + "/",
                        'r');
  } else if (config.type == "python2.7" || config.type == "python3") {
    soft_ban_file_name_set.insert(dirname(realpath(config.program_name)) +
                                  "/__pycode__/");
  ...
  readable_file_name_set.insert(writable_file_name_set.begin(),
                                writable_file_name_set.end());
  statable_file_name_set.insert(readable_file_name_set.begin(),
                                readable_file_name_set.end());
}
...
bool set_seccomp_bpf() {
  ...
  try {
    for (int no : supported_soft_ban_errno_list) {
      if (seccomp_rule_add(ctx, SCMP_ACT_ERRNO(no), SYSCALL_SOFT_BAN_MASK | no,
                           0) < 0) {
        throw system_error();
      }
    }
...
void rp_child_proc::set_error_for_kill() {
  ...
  reg_val_t sig =
      this->syscall == __NR_tgkill ? this->reg.REG_ARG2 : this->reg.REG_ARG1;
...
bool rp_child_proc::check_file_permission(const string &op, const string &fn,
                                          char mode) {
  string real_fn;
  if (!fn.empty()) {
  ...
  if (real_fn.compare(0, path_proc_self.size() + 1, path_proc_self + "/") ==
      0) {
  ...
  if (is_soft_ban_file(real_fn)) {
    ...
    this->set_error_for_suspicious(
        "intended to access a file without permission: " + op);
    return false;
  }
}
...
bool rp_child_proc::check_safe_syscall() {
  ...
  int cur_instruction =
      ptrace(PTRACE_PEEKTEXT, pid, reg.rip - 2, NULL) & 0xffff;
  if (cur_instruction != 0x050f) {
    if (run_program_config.need_show_trace_details) {
      fprintf(stderr, "informal syscall  %d\n", cur_instruction);
    }
    this->set_error_for_suspicious("incorrect opcode " +
                                   to_string(cur_instruction));
    return false;
  }

  if (0 > (long long int)reg.REG_SYSCALL ||
      (long long int)reg.REG_SYSCALL >= N_SYSCALL) {
  ...
  this->try_to_create_new_process =
      syscall == __NR_fork || syscall == __NR_clone || syscall == __NR_vfork;
  ...
  if (cursc.extra_check & ECT_FILE_OP) {
    ...
    if (cursc.extra_check & ECT_CHECK_OPEN_FLAGS) {
      reg_val_t flags =
          cursc.extra_check & ECT_END_AT ? reg.REG_ARG2 : reg.REG_ARG1;
  ...
  if (cursc.extra_check & ECT_CLONE_THREAD) {
    ...
    standard_flags |= CLONE_SYSVSEM | CLONE_SETTLS | CLONE_PARENT_SETTID |
                      CLONE_CHILD_CLEARTID;
    if (!(flags & standard_flags)) {
      this->set_error_for_suspicious(
          "intended to create a non-standard thread");
      return false;
    }
  }
