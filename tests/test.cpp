...
static fs::path init_random_tmp_dir(const string &src) {
  ...
  while (true) {
    ...
    if (fs::create_directory(path)) {
      main_path =
          fs::read_symlink("/proc/self/exe").parent_path().parent_path();
      work_path = path / "work";
      fs::copy(fs::path(main_path) / "tests" / src, work_path,
...
TEST_CASE("check result type of run_program", "[run_program]") {
  pair<string, runp::RS_TYPE> li[] = {
      {"hellocpp", runp::RS_AC}, {"killme", runp::RS_RE},
      {"div0", runp::RS_RE},     {"glibcerr", runp::RS_RE},
      {"newer", runp::RS_MLE},   {"oler", runp::RS_OLE},
      {"fork", runp::RS_DGS},
  };
    ...
    for (string ver : {/* "8", */ "11", "17"}) {
      SECTION(ts.first + "-java" + ver) {
        ...
        auto mycompile = [&](int i) {
          return [i, ts, ver] {
            file_copy(work_path + "/answer.code",
                      work_path + "/answer" + to_string(i) + ".code");
...
TEST_CASE("coin2 benchmark", "[interaction][benchmark]") {
  ...
  REQUIRE(execute("g++", "-o", "interactor", "interactor14.cpp",
                  "-I" + main_path + "/include", "-w") == 0);
  ...
  BENCHMARK("coin2 interaction") {
    auto res = run_simple_interaction("coin.in", "coin.out", "coin.real.in",
                                      "coin.real.out", lim, ilim, "std");
  ...
  BENCHMARK("coin2 raw interaction") {
    execute("./interactor", "coin.in", "/dev/stdin", "coin.out", "<fifo",
            "2>/dev/null", "|", "./std", ">fifo", "2>/dev/null");
  };
...
TEST_CASE("double to time structure test", "[time limit]") {
  for (int t = 0; t < 10000; t++) {
    ...
    REQUIRE(
        (spec.tv_sec == t / 1000 && spec.tv_nsec == (t % 1000) * 1'000'000));
  }
