...
sample resample(URng& rng, unsigned int resamples,
...
Config::Config(ConfigData const& data)
    : m_data(data), m_defaultStream(openStream(data.defaultOutputFilename)) {
  ...
  for (auto const& reporterAndFile : m_data.reporterSpecifications) {
    if (reporterAndFile.outputFileName.none()) {
      m_reporterStreams.emplace_back(
          new Detail::RDBufStream(m_defaultStream->stream().rdbuf()));
    } else {
      m_reporterStreams.emplace_back(
...
std::vector<ConfigData::ReporterAndFile> const&
Config::getReportersAndOutputFiles() const {
...
std::ostream& Config::defaultStream() const {
  return m_defaultStream->stream();
}
...
Detail::unique_ptr<IStream const> Config::openStream(
...
IStreamingReporterPtr makeReporter(Config const* config) {
  if (Catch::getRegistryHub().getReporterRegistry().getListeners().empty() &&
    ...
    return createReporter(config->getReportersAndOutputFiles()[0].reporterName,
                          ReporterConfig(config, stream));
  }
  ...
  for (auto const& listener : listeners) {
    multi->addListener(listener->create(
        Catch::ReporterConfig(config, config->defaultStream())));
  }
  ...
  for (auto const& reporterAndFile : config->getReportersAndOutputFiles()) {
    ...
    multi->addReporter(createReporter(reporterAndFile.reporterName,
                                      ReporterConfig(config, stream)));
    reporterIdx++;
  }
...
std::uint64_t Counts::total() const { return passed + failed + failedButOk; }
...
Clara::Parser makeCommandLineParser(ConfigData& config) {
  ...
  auto const setReporter = [&](std::string const& reporterSpec) {
    ...
    if (containsFileName) {
      auto nextSeparatorPos =
          reporterSpec.find(separator, fileNameSeparatorPos + separatorSize);
    ...
    if (containsFileName) {
      outputFileName = reporterSpec.substr(fileNameSeparatorPos + separatorSize,
                                           reporterSpec.size());
    }
    ...
    if (result == factories.end())
      return ParserResult::runtimeError(
          "Unrecognized reporter, '" + reporterName +
          "'. Check available with --list-reporters");
    if (containsFileName && outputFileName->empty())
      return ParserResult::runtimeError(
          "Reporter '" + reporterName +
          "' has empty filename specified as its output. Supply a filename or "
          "remove the colons to use the default output.");

    config.reporterSpecifications.push_back(
        {std::move(reporterName), std::move(outputFileName)});

    // It would be enough to check this only once at the very end, but there is
    // not a place where we could call this check, so do it every time it could
    // fail. For valid inputs, this is still called at most once.
    if (!containsFileName) {
  ...
  auto cli =
      ...
      Opt(accept_many, setReporter, "name[:output-file]")["-r"]["--reporter"](
...
void FatalConditionHandler::disengage_platform() {
  if (SetUnhandledExceptionFilter(
          reinterpret_cast<LPTOP_LEVEL_EXCEPTION_FILTER>(
              previousTopLevelExceptionFilter)) != topLevelExceptionFilter) {
    CATCH_RUNTIME_ERROR(
        "Could not restore previous top level exception filter");
  }
...
IStreamingReporterPtr ReporterRegistry::create(
    std::string const& name, ReporterConfig const& config) const {
...
Totals RunContext::runTest(TestCaseHandle const& testCase) {
  ...
  do {
    ...
    auto statsForOneRun = TestCaseStats(testInfo, singleRunTotals, oneRunCout,
                                        oneRunCerr, aborting());
...
void RunContext::handleMessage(AssertionInfo const& info,
                               ResultWas::OfType resultType, StringRef message,
                               AssertionReaction& reaction) {
  m_reporter->assertionStarting(info);
...
void printTotals(std::ostream& out, const Totals& totals) {
  if (totals.testCases.total() == 0) {
    ...
    Colour colour(Colour::ResultError);
    out << "Failed " << pluralise(totals.testCases.failed, "test case"_sr)
        << ", "
           "failed "
        << pluralise(totals.assertions.failed, "assertion"_sr) << '.';
  } else {
...
void CompactReporter::assertionEnded(AssertionStats const& _assertionStats) {
  ...
  if (!m_config->includeSuccessfulResults() && result.isOk()) {
    if (result.getResultType() != ResultWas::Warning) return;
...
void ConsoleReporter::assertionEnded(AssertionStats const& _assertionStats) {
  ...
  if (!includeResults && result.getResultType() != ResultWas::Warning) return;
...
void ConsoleReporter::lazyPrintRunInfo() {
  ...
  m_stream << currentTestRunInfo.name << " is a Catch v" << libraryVersion()
           << " host application.\n"
...
void ConsoleReporter::printSummaryRow(StringRef label,
  ...
  for (auto col : cols) {
    ...
    if (col.label.empty()) {
      ...
      m_stream << Colour(col.colour) << value << ' ' << col.label;
    }
  }
  m_stream << '\n';
}
...
void ConsoleReporter::printTotalsDivider(Totals const& totals) {
  if (totals.testCases.total() > 0) {
    ...
    m_stream << Colour(Colour::ResultExpectedFailure)
             << std::string(failedButOkRatio, '=');
    if (totals.testCases.allPassed())
      m_stream << Colour(Colour::ResultSuccess)
               << std::string(passedRatio, '=');
    else
      m_stream << Colour(Colour::Success) << std::string(passedRatio, '=');
  } else {
    m_stream << Colour(Colour::Warning)
...
void CumulativeReporterBase::listReporters(
    std::vector<ReporterDescription> const& descriptions) {
  defaultListReporters(m_stream, descriptions, m_config->verbosity());
}

void CumulativeReporterBase::listTests(
    std::vector<TestCaseHandle> const& tests) {
  defaultListTests(m_stream, tests, m_config->hasTestFilters(),
                   m_config->verbosity());
}
...
JunitReporter::JunitReporter(ReporterConfig const& _config)
    : CumulativeReporterBase(_config), xml(m_stream) {
...
namespace Catch {
void ListeningReporter::updatePreferences(
    IStreamingReporter const& reporterish) {
...
void ListeningReporter::testCasePartialStarting(TestCaseInfo const& testInfo,
                                                uint64_t partNumber) {
  for (auto& reporterish : m_reporterLikes) {
...
void StreamingReporterBase::listReporters(
    std::vector<ReporterDescription> const& descriptions) {
  defaultListReporters(m_stream, descriptions, m_config->verbosity());
}

void StreamingReporterBase::listTests(
    std::vector<TestCaseHandle> const& tests) {
  defaultListTests(m_stream, tests, m_config->hasTestFilters(),
                   m_config->verbosity());
}
