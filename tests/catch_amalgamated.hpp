...
struct BenchmarkStats {
  ...
  operator BenchmarkStats<Duration2>() const {
    ...
    return {
        info,     CATCH_MOVE(samples2), mean, standardDeviation,
...
TimingOf<Clock, Fun, run_for_at_least_argument_t<Clock, Fun>> run_for_at_least(
    ClockDuration<Clock> how_long, const int initial_iterations, Fun&& fun) {
...
#pragma clang diagnostic pop
  ...
  env = Catch::Detail::make_unique<Environment<FloatDuration<Clock>>>(
      Environment<FloatDuration<Clock>>{resolution, cost});
  return *env;
}
}  // namespace Detail
}  // namespace Benchmark
...
struct SampleAnalysis {
  ...
  operator SampleAnalysis<Duration2>() const {
    ...
    return {
        CATCH_MOVE(samples2), mean, standard_deviation, outliers,
        outlier_variance,
    };
  }
};
...
SampleAnalysis<Duration> analyse(const IConfig& cfg, Environment<Duration>,
  ...
  if (!cfg.benchmarkNoAnalysis()) {
    ...
    return {CATCH_MOVE(samples), Estimate<Duration>{mean, mean, mean, 0.0},
...
namespace Benchmark {
struct Benchmark {
  Benchmark(std::string&& benchmarkName) : name(CATCH_MOVE(benchmarkName)) {}
...
struct StringMaker<char[SZ]> {
  static std::string convert(char const* str) {
    ...
    return Detail::convertIntoString(StringRef(str, strnlen(str, SZ)));
  }
};
template <size_t SZ>
...
struct ConfigData {
  struct ReporterAndFile {
    ...
    friend bool operator==(ReporterAndFile const& lhs,
                           ReporterAndFile const& rhs) {
      return lhs.reporterName == rhs.reporterName &&
             lhs.outputFileName == rhs.outputFileName;
    }
    friend std::ostream& operator<<(std::ostream& os,
                                    ReporterAndFile const& reporter);
  };
...
class Config : public IConfig {
 public:
  ...
  std::vector<ConfigData::ReporterAndFile> const& getReportersAndOutputFiles()
      const;
 ...
 private:
  Detail::unique_ptr<IStream const> openStream(
...
struct is_unary_function<
    F, Catch::Detail::void_t<decltype(std::declval<F>()(fake_arg()))>>
    : std::true_type {};
...
class Arg : public Detail::ParserRefImpl<Arg> {
 public:
  ...
  Detail::InternalParseResult parse(
      std::string const&, Detail::TokenStream const& tokens) const override;
};
...
struct Decomposer {
  template <
      typename T,
      std::enable_if_t<!std::is_arithmetic<std::remove_reference_t<T>>::value,
                       int> = 0>
...
#elif !defined(CATCH_CONFIG_PREFIX_ALL) && \
    defined(CATCH_CONFIG_DISABLE)  // ^^ unprefixed, implemented | vv
                                   // unprefixed, disabled
...
struct TestCaseInfo : Detail::NonCopyable {
  TestCaseInfo(StringRef _className, NameAndTags const& _tags,
...
Detail::unique_ptr<TestCaseInfo> makeTestCaseInfo(
    StringRef className, NameAndTags const& nameAndTags,
    SourceLineInfo const& lineInfo);
}  // namespace Catch
...
class RunContext : public IResultCapture {
 ...
 public:  // IResultCapture
  ...
  void handleMessage(AssertionInfo const& info, ResultWas::OfType resultType,
                     StringRef message, AssertionReaction& reaction) override;
...
class StreamingReporterBase : public IStreamingReporter {
 public:
  StreamingReporterBase(ReporterConfig const& _config)
      : IStreamingReporter(_config.fullConfig()), m_stream(_config.stream()) {}
...
struct ConsoleReporter final : StreamingReporterBase {
 ...
 private:
  ...
  void printSummaryRow(StringRef label, std::vector<SummaryColumn> const& cols,
                       std::size_t row);
...
class CumulativeReporterBase : public IStreamingReporter {
 ...
 protected:
  //! Should the cumulative base store the assertion expansion for succesful
  //! assertions?
...
class ListeningReporter final : public IStreamingReporter {
 ...
 public:
  ListeningReporter(IConfig const* config) : IStreamingReporter(config) {}
...
struct SonarQubeReporter final : CumulativeReporterBase {
  SonarQubeReporter(ReporterConfig const& config)
      : CumulativeReporterBase(config), xml(m_stream) {
