...
PointInfo submit_answer_test_point_with_auto_generation(
    int i, TestPointConfig tpc = TestPointConfig()) {
  ...
  tpc.output_file_name =
      work_path + "/" + conf_output_file_name(conf_int("output_file_id", i, i));
...
void ordinary_test() {
  if (conf_is("submit_answer", "on")) {
    ...
    primary_data_test(
        [](int i) { return submit_answer_test_point_with_auto_generation(i); },
        pdtc);
  } else {
    ...
    primary_data_test([](int i) { return test_point("answer", i); });
  }
  end_judge_ok();
}
...
void sample_test() {
  if (conf_is("submit_answer", "on")) {
    ...
    sample_data_test([](int i) { return test_point("answer", i); });
    end_judge_ok();
  }
}
